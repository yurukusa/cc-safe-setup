...
| I want to... | Command |
|---|---|
| Install basic safety | `npx github:yurukusa/cc-safe-setup` |
| Maximum protection | `npx github:yurukusa/cc-safe-setup --shield` |
| Check my setup | `npx github:yurukusa/cc-safe-setup --doctor` |
| See my safety score | `npx github:yurukusa/cc-safe-setup --audit` |
...
```bash
npx github:yurukusa/cc-safe-setup --install-example block-database-wipe
```
Blocks: `prisma migrate reset`, `rails db:drop`, `DROP TABLE`, etc.

### Block npm publish accidents
```bash
npx github:yurukusa/cc-safe-setup --install-example npm-publish-guard
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example auto-approve-git-read
```
...
```bash
npx github:yurukusa/cc-safe-setup --protect .env
```

### Protect CLAUDE.md from unauthorized changes
```bash
npx github:yurukusa/cc-safe-setup --install-example protect-claudemd
```

### Protect dotfiles (~/.bashrc, ~/.aws/)
```bash
npx github:yurukusa/cc-safe-setup --install-example protect-dotfiles
```
...
```bash
npx github:yurukusa/cc-safe-setup --rules rules.yaml
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example auto-compact-prep
```

### Track context window usage
```bash
npx github:yurukusa/cc-safe-setup --install-example compact-reminder
```

### Fix hook permissions on Windows/plugins
```bash
npx github:yurukusa/cc-safe-setup --install-example hook-permission-fixer
```

### Prevent tool call loops
```bash
npx github:yurukusa/cc-safe-setup --install-example response-budget-guard
```
...
```bash
npx github:yurukusa/cc-safe-setup --test-hook destructive-guard
```

### Preview how hooks react to a command
```bash
npx github:yurukusa/cc-safe-setup --simulate "git push --force origin main"
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example allow-git-hooks-dir
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example credential-exfil-guard
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example rm-safety-net
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example auto-mode-safe-commands
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example compound-command-allow
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example write-secret-guard
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example permission-audit-log
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example classifier-fallback-allow
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example credential-file-cat-guard
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example push-requires-test-pass
npx github:yurukusa/cc-safe-setup --install-example push-requires-test-pass-record
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example github-actions-secret-guard
npx github:yurukusa/cc-safe-setup --install-example ci-workflow-guard
npx github:yurukusa/cc-safe-setup --install-example gitops-drift-guard
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example k8s-production-guard
...
```bash
npx github:yurukusa/cc-safe-setup --install-example mcp-server-allowlist
npx github:yurukusa/cc-safe-setup --install-example mcp-tool-audit-log
...
```bash
npx github:yurukusa/cc-safe-setup --install-example role-tool-guard
...
```bash
npx github:yurukusa/cc-safe-setup --install-example git-show-flag-sanitizer
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example session-backup-on-start
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example pre-compact-transcript-backup
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example compact-blocker
```
...
```bash
npx github:yurukusa/cc-safe-setup --install-example webfetch-domain-allow
```
