...
**Claude Codeを安全にするワンコマンドツール。** 667個のexample hook · 9,200+テスト · 1,200+ インストール/週
...
> 📘 トークン消費が多すぎる？ [Token Book](token-book.html)（¥2,500）でCLAUDE.md最適化・hookによるトークン制御・コンテキスト管理・ワークフロー設計を解説。800+時間の実測データ付き。第1章無料。hookの設計パターンは[Safety Guide](https://zenn.dev/yurukusa/books/6076c23b1cb18b)（¥800・第3章まで無料）。
