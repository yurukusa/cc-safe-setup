...
test_ex destructive-guard.sh '{"tool_input":{"command":"echo rm | bash"}}' 0 "destructive-guard: pipe to bash (not caught - echo only)"
test_ex destructive-guard.sh '{"tool_input":{"command":"rm -rf / --no-preserve-root"}}' 2 "destructive-guard: --no-preserve-root bypass blocked"
test_ex destructive-guard.sh '{"tool_input":{"command":"  rm   -rf   /"}}' 2 "destructive-guard: extra whitespace bypass blocked"
test_ex destructive-guard.sh '{"tool_input":{"command":"rm -r -f /"}}' 2 "destructive-guard: split flags bypass blocked"
test_ex destructive-guard.sh '{"tool_input":{"command":"sudo rm -rf /"}}' 2 "destructive-guard: sudo prefix bypass blocked"
test_ex destructive-guard.sh '{"tool_input":{"command":"rm -rf ~"}}' 2 "destructive-guard: tilde home bypass blocked"
test_ex branch-guard.sh '{"tool_input":{"command":"git push origin HEAD:main"}}' 2 "branch-guard: HEAD:main bypass blocked"
test_ex branch-guard.sh '{"tool_input":{"command":"git push origin HEAD:refs/heads/main"}}' 2 "branch-guard: refs/heads/main bypass blocked"
test_ex branch-guard.sh '{"tool_input":{"command":"git push --force origin main"}}' 2 "branch-guard: --force flag bypass blocked"
test_ex branch-guard.sh '{"tool_input":{"command":"git push -f origin main"}}' 2 "branch-guard: -f shortflag bypass blocked"
test_ex secret-guard.sh '{"tool_input":{"new_string":"AKIAIOSFODNN7EXAMPLE"}}' 2 "secret-guard: AWS key in new_string blocked"
test_ex secret-guard.sh '{"tool_input":{"content":"sk-proj-abcdef1234567890"}}' 2 "secret-guard: OpenAI key in content blocked"
test_ex secret-guard.sh '{"tool_input":{"new_string":"ghp_1234567890abcdef1234567890abcdef12345678"}}' 2 "secret-guard: GitHub PAT blocked"
test_ex secret-guard.sh '{"tool_input":{"new_string":"password = hunter2"}}' 2 "secret-guard: password assignment blocked"
test_ex secret-guard.sh '{"tool_input":{"new_string":"const apiKey = process.env.KEY"}}' 0 "secret-guard: env var reference allowed"
...
test_ex deploy-guard.sh '{"tool_input":{"command":"kubectl apply -f deploy.yaml"}}' 2 "deploy-guard: kubectl apply blocked"
test_ex deploy-guard.sh '{"tool_input":{"command":"terraform apply -auto-approve"}}' 2 "deploy-guard: terraform apply blocked"
