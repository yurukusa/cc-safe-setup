...
# --- the rules two directories up still bind ---------------------------------
# Claude Code walks up to find CLAUDE.md. Before this was fixed, running the
# audit from a subdirectory of your own project reported "No CLAUDE.md found"
# and silently dropped every cross-layer finding with it.
check_subdir() {
  local home proj out
  home=$(mktemp -d); proj=$(mktemp -d)
  mkdir -p "$home/.claude/hooks" "$proj/src/deep"
  printf '%s' '{"hooks":{"PreToolUse":[]}}' > "$home/.claude/$SETTINGS_FILE"
  printf '#!/usr/bin/env bash\nexit 0\n' > "$home/.claude/hooks/subdir-guard.sh"
  printf -- '- force push is stopped by `subdir-guard.sh`\n' > "$proj/CLAUDE.md"
  out=$(cd "$proj/src/deep" && HOME="$home" node "$ROOT/index.mjs" --audit 2>&1)
  rm -rf "$home" "$proj"
  if printf '%s' "$out" | grep -q 'No CLAUDE.md found'; then
    echo "  FAIL: a CLAUDE.md two directories up is found (reported as missing)"
    FAIL=$((FAIL + 1)); return
  fi
  if ! printf '%s' "$out" | grep -q 'appear in no settings file'; then
    echo "  FAIL: a rule two directories up is still read (finding was dropped)"
    FAIL=$((FAIL + 1)); return
  fi
  echo "  PASS: a CLAUDE.md two directories up is found and its rules are read"
  PASS=$((PASS + 1))
}
check_subdir
