...
# The splitter only knew && ; and ||, so a read-only command piped into a git
# write arrived as ONE segment whose first word was the reader. READ_ONLY_RE
# exempted the whole thing and the write on the right of the pipe was never
# examined. Measured on the published file: these four exited 0.
...
# shows up green on the blocking table while quietly breaking normal work,
# so the passing side is listed here on purpose.
