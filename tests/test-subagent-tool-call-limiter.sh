...
# ★2026-08-27に期待値を 15 → 10 へ変えた。挙動の変更が意図的なため。
#   記録を遮断の判定より後ろへ移したので、遮断された呼び出しはカウンターを進めない。
#   前は遮断されるたびに 11,12,…15 と増え、案内を読んで再試行するほど数字が悪化した。
#   この試験が守りたい「呼び出しをまたいで貯まる」(修正前は毎回1)は 10 でも守られている。
assert_eq "カウンターが上限で止まる(遮断された5回は加算されない)" "$(cat "$CF" 2>/dev/null)" "10"

# 回帰2b: 遮断された状態で何度叩いてもカウンターが動かない(再試行で待ちが延びない)
before2b=$(cat "$CF" 2>/dev/null)
for i in 1 2 3; do printf '%s' "$J" | CC_MAX_TOOL_CALLS=10 bash "$HOOK" >/dev/null 2>&1; done
assert_eq "遮断中の再試行はカウンターを動かさない" "$(cat "$CF" 2>/dev/null)" "$before2b"
