...
- **`warn-cron-cost-trap.sh`: warn on short-interval recurring CronCreate (#74547)** —
  ...
  smallest consecutive gap with 60-wraparound, single/range → hourly — and, only when
  ...
  `tests/test-warn-cron-cost-trap.sh` (19 assertions). Pairs with `cron-create-receipt.sh`.
