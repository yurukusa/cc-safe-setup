...
# then handed to the entire line: `mvn test && sudo rm -rf app` was approved on
...
if cc_every_segment_matches '^\s*(mvn|mvnw|./mvnw)\s+(compile|test|verify|package|clean|install)(\s|$)' "$COMMAND"; then
