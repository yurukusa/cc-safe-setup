...
# then handed to the entire line: `make test && sudo rm -rf app` was approved on
# the strength of its first word. Measured 2026-08-03. Same defect as
...
if cc_every_segment_matches '^\s*make\s+(build|test|lint|format|check|clean|install|all|dev|start|run)(\s|$)' "$COMMAND"; then
