...
# then handed to the entire line: `go test ./... && sudo rm -rf app` was
...
if cc_every_segment_matches '^\s*go\s+(build|test|vet|fmt|mod|run|generate|install|clean)(\s|$)' "$COMMAND"; then
