...
if ! command -v jq >/dev/null 2>&1; then
  _nojq_warned="/tmp/cc-nojq-warned-forbidden-response-phrase-detector-$PPID"
  [ -f "$_nojq_warned" ] || {
    echo "WARNING [forbidden-response-phrase-detector]: jq not found - this hook cannot inspect tool calls and is NOT protecting you. Install jq." >&2
    : > "$_nojq_warned"
  }
fi
