...
# then handed to the entire line: `cargo build && sudo rm -rf app` was approved
# on the strength of its first word. Measured 2026-08-03 against the shipped
# copy. Same defect as PR #937 (allowlist.sh) and PR #940 (cd-git-allow.sh),
# here on the approving side, where the decision is an explicit approval rather
# than a missed block.
#
...
if cc_every_segment_matches '^\s*cargo\s+(build|test|check|clippy|fmt|run|bench|doc|clean)(\s|$)' "$COMMAND"; then
